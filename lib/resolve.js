...
const getImportTransformer = objKeyCache((aliases) => stringKeyCache((
  importPath,
  basePath,
  extensions
) => {
  ...
  for (const transformer of transformers) {
    ...
    return checkImports.find((curImport, i) => (
      checkExtensions.some((ext) => {
        const {
          exists,
          isDirectory
        } = checkFile(curImport + ext)
        ...
        return checkImports.length > 1 && ext === '' && i === 0
          ? exists && !isDirectory
          : exists
      })
    ))
...
const resolvePath = (sourceFile, importPath, basePath, extensions, aliases, relative) => {
  ...
  const transformImport = getImportTransformer(aliases)
  const validImport = transformImport(
    importPath,
    basePath,
    extensions
  )
